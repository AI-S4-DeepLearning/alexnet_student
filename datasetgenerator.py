...
class DatasetGenerator:
    ...
    def get_balanced_splits(self, train_ratio: float, val_ratio: float, min_class_size: int):
        ...
        # De "gebalanceerde" datasets zijn eigenlijk nog gewoon BasicDatasets, ongebalanceerd dus. 
        # Pas hier de gebalanceerde dataset toe.
        train_dataset = BasicDataset(train_ds, self.img_size)
        train_dataset = BalancedDataset(train_ds, min_class_size, self.classes, self.img_size)                        # <XXX>
...
class BalancedDataset(Dataset):                                                                                                # <XXX
    """
    Dataset wrapper that oversamples and augments classes with fewer
    samples than a specified target size.

    Augmented samples receive random transformations to increase
    diversity, while original samples remain unmodified except for
    resizing and normalization.

    Attributes:
        base_dataset (Dataset): The original dataset to wrap.
        classes (list[str]): List of class names.
        samples (list[tuple[int, bool]]): List of (index, is_augmented) pairs
            defining the oversampled dataset.
        transform (callable): Transformation pipeline applied to images.
    """
    def __init__(self, base_dataset: Dataset, min_class_size: int, classes: list[str], img_size: int):
        ...
        for _, indices in class_indices.items():
            ...
            if original_count < min_class_size:
                needed = min_class_size - original_count
                aug_indices = np.tile(indices, (needed // original_count) + 1)[:needed]
    ...
    def __getitem__(self, idx: int):
        ...
        """
        base_idx, is_augmented = self.samples[idx]
